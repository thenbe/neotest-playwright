...
____exports._build_position = function(filePath, source, capturedNodes)
    ...
    elseif match_type == "test" then
        local base = {type = match_type, range = range, path = filePath}
        local position = buildTestPosition(base, name)
        return position
    else
        error(
            __TS__New(Error, "Unknown match type"),
...
buildTestPosition = function(position, name)
    return {
        __TS__ObjectAssign({}, position, {name = name .. "-wrange"}),
        __TS__ObjectAssign({}, position, {name = name .. "-2", is_sterile = true}),
        __TS__ObjectAssign({}, position, {name = name .. "-3", is_sterile = true})
    }
end
return ____exports
